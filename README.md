...
![Version](https://img.shields.io/badge/Version-2.4.2.3.2-green.svg)
...
```bash
curl -L https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-mac-arm64.zip -o ~/Downloads/Kuiqr.zip && unzip -o ~/Downloads/Kuiqr.zip -d /Applications && xattr -cr /Applications/Kuiqr.app && open /Applications/Kuiqr.app
```
...
```bash
curl -L https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-mac-x64.zip -o ~/Downloads/Kuiqr.zip && unzip -o ~/Downloads/Kuiqr.zip -d /Applications && xattr -cr /Applications/Kuiqr.app && open /Applications/Kuiqr.app
```
...
```powershell
Invoke-WebRequest -Uri "https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-windows-x64-setup.exe" -OutFile "$env:USERPROFILE\Downloads\Kuiqr-setup.exe"; Start-Process "$env:USERPROFILE\Downloads\Kuiqr-setup.exe"
```
...
```bash
# x64
curl -L https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-x86_64.AppImage -o ~/Downloads/Kuiqr.AppImage && chmod +x ~/Downloads/Kuiqr.AppImage && ~/Downloads/Kuiqr.AppImage
# arm64
curl -L https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-arm64.AppImage -o ~/Downloads/Kuiqr.AppImage && chmod +x ~/Downloads/Kuiqr.AppImage && ~/Downloads/Kuiqr.AppImage
```

> **Note:** Kuiqr runs in the background and lives in your system tray. On GNOME you may need the [AppIndicator](https://extensions.gnome.org/extension/615/appindicator-support/) extension for the tray icon to appear. A `.deb` package is also provided for Debian/Ubuntu-based distros (`Kuiqr-2.4.2.3.2-linux-{x64,arm64}.deb`).

### Chrome / Edge / Brave Extension

```bash
# Download and extract
curl -L https://github.com/LarryXu2014/Kuiqr/releases/latest/download/kuiqr-extension-2.4.2.3.2.zip -o /tmp/kuiqr-extension-2.4.2.3.2.zip && unzip -q /tmp/kuiqr-extension-2.4.2.3.2.zip -d ~/kuiqr-extension && echo "Extension extracted to ~/kuiqr-extension — load it from chrome://extensions > Load unpacked"
```
...
curl -L "https://ghproxy.com/https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-mac-arm64.zip" -o ~/Downloads/Kuiqr.zip && unzip -o ~/Downloads/Kuiqr.zip -d /Applications && open /Applications/Kuiqr.app
```
...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ |
| Chrome / Edge / Brave      | [kuiqr-extension-2.4.2.3.2.zip](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/kuiqr-extension-2.4.2.3.2.zip)                           | 84 KB | Verified |
| Firefox (109+)             | [kuiqr-firefox-2.4.2.3.2.zip](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/kuiqr-firefox-2.4.2.3.2.zip)                               | 84 KB | Verified |
| macOS (Apple Silicon)      | [Kuiqr-2.4.2.3.2-mac-arm64.zip](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-mac-arm64.zip)     | ~92 MB | Verified |
...
| Linux (x64, AppImage)      | [Kuiqr-2.4.2.3.2-linux-x86_64.AppImage](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-x86_64.AppImage) | ~90 MB | Verified |
| Linux (arm64, AppImage)    | [Kuiqr-2.4.2.3.2-linux-arm64.AppImage](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-arm64.AppImage) | ~90 MB | Verified |
| Linux (x64, .deb)          | [Kuiqr-2.4.2.3.2-linux-amd64.deb](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-amd64.deb) | ~90 MB | Verified |
| Linux (arm64, .deb)        | [Kuiqr-2.4.2.3.2-linux-arm64.deb](https://github.com/LarryXu2014/Kuiqr/releases/latest/download/Kuiqr-2.4.2.3.2-linux-arm64.deb) | ~90 MB | Verified |
...
1. Download `kuiqr-extension-2.4.2.3.2.zip` and unzip it
...
1. Download `kuiqr-firefox-2.4.2.3.2.zip` and unzip it
...
### v2.4.2.3.2
- **Fixed "Could not check for updates" crash** — the cached update result was declared `const` and couldn't be reassigned, throwing on every check. Now `let`, so updates work again.
- **Offline-aware startup check** — on launch Kuiqr probes for connectivity first and silently skips the update check when offline (no false error).
- **Compact updates bar** — the Settings → Updates row is now a normal-height button instead of an oversized bar.
- **Correct menu-bar icon** — macOS uses a dedicated monochrome template tray icon (crisp in light & dark) instead of the wrong/colored asset.
- **First-launch onboarding fixed** — a fresh install now shows the Welcome → Language → Browser extension → Guided tour → Done flow and tucks into the menu bar when done, instead of closing.
- **Tour follows the language** — the guided tour now re-localizes its text to the chosen language on every run.
- **Language change restarts the app** — switching language in Settings or the setup wizard relaunches Kuiqr so the new locale applies everywhere (no leftover English UI).

### v2.4.2.3.1
- **Fixed macOS `.dmg` "disk image is corrupted" download error** — re-published the macOS dmg with `internetEnabled: true` (multi-segment UDIF format). Safari/Chrome now treat the dmg like a normal downloadable file, so a partial or interrupted download no longer trips macOS into misinterpreting the file as corrupt. Volume label is now the actual build version (`Kuiqr 2.4.2.3.2`), matching the published release. **If you hit "corrupted" on v2.4.2.3 — just re-download the dmg, or use the `.zip` which is unaffected.**
...
cd extension && zip -r ../kuiqr-extension-2.4.2.3.2.zip . -x "*.DS_Store"
...
cd /tmp/ff-build && zip -r /path/to/kuiqr-firefox-2.4.2.3.2.zip . -x "*.DS_Store"
```
