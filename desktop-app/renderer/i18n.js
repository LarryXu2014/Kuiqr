...
const I18N = {
  ...
  "gen.dynamicDesc": { en: "Encode a redirect link so you can see scan analytics. Requires a backend set in Settings." , "zh-CN": "编码一个跳转链接以查看扫描统计。需要在设置中配置后端。", "zh-TW": "編碼一個轉址連結以查看掃描統計。需要在設定中配置後端。", "ja": "リダイレクトリンクをエンコードしてスキャン解析を確認できます。設定でバックエンドの設定が必要です。", "ko": "리디렉션 링크를 인코딩하여 스캔 통계를 확인합니다. 설정에서 백엔드를 구성해야 합니다.", "es": "Codifica un enlace de redirección para ver estadísticas de escaneo. Requiere un backend configurado en Ajustes.", "fr": "Encode un lien de redirection pour voir les statistiques de scan. Nécessite un backend défini dans les Paramètres.", "de": "Codiert einen Weiterleitungs-Link, um Scan-Statistiken zu sehen. Erfordert ein Backend in den Einstellungen." },
  ...
  "gen.dynamicNeedsUrl": { en: "Trackable QR only works with web links (http/https)." , "zh-CN": "可追踪二维码仅支持网页链接（http/https）。", "zh-TW": "可追蹤 QR Code 僅支援網頁連結（http/https）。", "ja": "追跡可能QRコードはWebリンク（http/https）でのみ動作します。", "ko": "추적 가능 QR 코드는 웹 링크(http/https)에서만 작동합니다.", "es": "El QR rastreable solo funciona con enlaces web (http/https).", "fr": "Le QR traçable ne fonctionne qu'avec des liens web (http/https).", "de": "Trackbare QR-Codes funktionieren nur mit Weblinks (http/https)." },
  ...
  "gen.dynamicHint": { en: "Turn on to encode a redirect link so you can track scans. No backend yet? Set up a free local one in Settings → Dynamic QR." , "zh-CN": "开启以编码跳转链接来追踪扫描。还没有后端？在 设置 → 动态二维码 免费搭建一个本地后端。", "zh-TW": "開啟以編碼轉址連結來追蹤掃描。還沒有後端？在 設定 → 動態 QR Code 免費架設一個本機後端。", "ja": "オンにするとリダイレクトリンクをエンコードしてスキャンを追跡します。バックエンドがまだない場合は、設定 → 動的QRコード で無料のローカルバックエンドをセットアップできます。", "ko": "켜면 리디렉션 링크를 인코딩하여 스캔을 추적합니다. 백엔드가 없나요? 설정 → 동적 QR에서 무료 로컬 백엔드를 설정하세요.", "es": "Actívalo para codificar un enlace de redirección y rastrear escaneos. ¿Sin backend? Configura uno local gratis en Ajustes → QR dinámico.", "fr": "Activez pour encoder un lien de redirection et suivre les scans. Pas de backend ? Configurez-en un local gratuit dans Paramètres → QR dynamique.", "de": "Aktivieren, um einen Weiterleitungs-Link zu codieren und Scans zu verfolgen. Noch kein Backend? Richten Sie ein kostenloses lokales unter Einstellungen → Dynamischer QR-Code ein." },
