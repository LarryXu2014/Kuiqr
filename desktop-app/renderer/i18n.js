...
const I18N = {
  ...
  "updates.status.open": { en: "Opening the installer… (finish the update from your Downloads folder if needed)", "zh-CN": "正在打开安装程序…（如有需要，请从下载文件夹完成更新）", "zh-TW": "正在開啟安裝程式…（如有需要，請從下載資料夾完成更新）", ja: "インストーラを開いています…（必要に応じてダウンロードフォルダから完了してください）", ko: "설치 프로그램을 여는 중…（필요하면 다운로드 폴더에서 마무리하세요）", es: "Abriendo el instalador… (completa la actualización desde tu carpeta de Descargas si es necesario)", fr: "Ouverture de l'installateur… (termine la mise à jour depuis votre dossier Téléchargements si besoin)", de: "Installer wird geöffnet… (schließe die Aktualisierung ggf. aus dem Downloads-Ordner ab)" },
